...
class HomeListView(APIView):
    ...
    def get(self, request):
        created_movies = (MoviesModel.objects.filter(is_active=True)[:5])
        double_movies = (MoviesModel.objects.filter(is_active=True, is_double=True)[:5])
        ...
        return Response({
            ...
            'double_movies': MoviesListSerializer(double_movies, many=True, context={'request': request}).data,
